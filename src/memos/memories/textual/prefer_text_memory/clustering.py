...
class HDBSCANClusterer(BaseClusterer):
    ...
    def __init__(self, min_cluster_size: int = 2, min_samples: int = 1, metric: str = "cosine"):
        ...
        self.clusterer = HDBSCAN(
            min_cluster_size=self.min_cluster_size, min_samples=self.min_samples, metric=self.metric
        )
    ...
    def search_knn_by_center_embeddings(
        self, center_emb: list[float], vectors: list[list[float]], top_k: int = 5
    ) -> list[dict[str, Any]]:
        """
