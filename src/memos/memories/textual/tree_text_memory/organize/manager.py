...
class MemoryManager:
    ...
    def replace_working_memory(self, memories: list[TextualMemoryItem], user_name: str = None) -> None:
        ...
        with ContextThreadPoolExecutor(max_workers=8) as executor:
            futures = [
                executor.submit(self._add_memory_to_db, memory, "WorkingMemory", user_name=user_name)
        ...
        self.graph_store.remove_oldest_memory(
            memory_type="WorkingMemory", keep_latest=self.memory_size["WorkingMemory"], user_name=user_name
        )
        self._refresh_memory_size(user_name=user_name)
