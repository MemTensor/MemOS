...
from memos.api.product_models import (
    UserRegisterRequest, GetMemoryRequest, ChatRequest, MemoryCreateRequest, SearchRequest,
    SimpleResponse, UserRegisterResponse, MemoryResponse, SuggestionResponse, SearchResponse, BaseResponse
)
...
async def register_user(user_req: UserRegisterRequest):
    ...
    try:
        ...
        user_config, default_mem_cube = APIConfig.create_user_config(user_name=user_req.user_id, user_id=user_req.user_id)
...
@router.get("/suggestions/{user_id}", summary="Get suggestion queries", response_model=SuggestionResponse)
async def get_suggestion_queries(user_id: str):
    ...
    try:
        ...
        return SuggestionResponse(message="Suggestions retrieved successfully", data={"query": suggestions})
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(traceback.format_exc()))
    except Exception as e:
...
@router.get("/configure/{user_id}", summary="Get MOSProduct configuration", response_model=SimpleResponse)
...
@router.get("/users/{user_id}/config", summary="Get user configuration", response_model=BaseResponse[dict])
async def get_user_config(user_id: str):
    ...
    try:
        ...
        if config:
            ...
            raise HTTPException(status_code=404, detail=f"Configuration not found for user {user_id}")
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(traceback.format_exc()))
    except Exception as e:
...
@router.put("/users/{user_id}/config", summary="Update user configuration", response_model=SimpleResponse)
