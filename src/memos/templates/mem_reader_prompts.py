...
Please perform:
1. Organize the document chunk into useful retrieval memories using the smallest complete record as the unit. The memories should be minimally redundant and collectively cover the important source information.
   - A memory should answer one natural question on its own. Keep attributes, qualifiers, values, and relationships that belong to the same record together.
   - Split content when it contains distinct subjects, rules, conditions, decisions, events, or data records that are independently useful for retrieval.
   - But do not split one fact, rule, record, or short list into fragments merely because it contains multiple fields or peer items.
2. Treat tables, lists, statistics, measurements, prices, dates, percentages, rankings, identifiers, and other structured or numerical content as high-priority information.
   - Preserve every row and the relationship between its row label and column values, including units, currencies, signs, ranges, and original category or rank names.
   - For a data table, normally create one memory per complete row and include the row label plus all related column values in that memory. Do not split one row into separate memories for individual cells or columns.
   - Keep a simple list of peer items together when the items share one subject and have no item-specific conditions, values, or explanations. Split list entries only when they carry independently meaningful details.
   - Never replace exact values or lower rows with vague summaries such as "and so on", "decreasing progressively", or "other entries are similar".
   - Do not calculate, normalize, rename, reorder, or "correct" values and labels unless the source explicitly provides the correction.
3. Identify key information that reflects factual content, insights, decisions, or implications from the documents — including any notable themes, conclusions, or data points. Allow a reader to fully understand the essence of the chunk without reading the original text.
4. Resolve all time, person, location, and event references clearly:
   ...
   - Never invent a missing year or date. Use document-level context only when it unambiguously identifies the same event or subject.
   - Include specific locations if mentioned.
   - Resolve all pronouns, aliases, and ambiguous references into full names or identities.
   - Disambiguate entities with the same name if applicable.
5. Always write from a third-person perspective, referring to the subject or content clearly rather than using first-person ("I", "me", "my").
6. Do not omit any information that is likely to be important or memorable from the document summaries.
  ...
  "memory list": [
    {
      ...
      "memory_type": "LongTermMemory",
      "value": <A clear, accurate, and self-contained smallest complete record, preserving closely related fields, exact structured values, and their relationships — written in English if the input memory items are in English, or in Chinese if the input is in Chinese>,
      "tags": <A list of relevant thematic keywords (e.g., ["deadline", "team", "planning"])>
    }
    ...
  ],
...
1. 以“最小完整记录”为单位，将文档片段组织成便于检索的多条记忆；各条记忆应尽量不重复，并合起来覆盖原文中的重要信息。
   ...
   - 当内容涉及不同主题、规则、条件、决策、事件或数据记录，并且分别具有独立检索价值时，需要进行拆分。
   - 但不要仅仅因为一个事实、规则、记录或简短列表包含多个字段或并列项，就把它机械拆成多个片段。
2. 对表格、列表、统计数据、计量值、价格、日期、百分比、排名、编号以及其他结构化或数值信息给予额外关注。
   - 完整保留每一行，以及行名称与各列数值之间的对应关系，包括单位、币种、正负号、范围和原始类别或名次名称。
   - 对数据表格，通常每一行生成一条记忆，并在该条记忆中同时保留行名称及所有相关列值；不得把同一行拆成多条记忆，分别保存不同单元格或列。
   - 当普通并列列表中的各项属于同一主题，且没有各自独立的条件、数值或解释时，普通并列列表应按共同主题合并为一条记忆；只有列表项包含可独立检索的重要细节时才拆分。
   - 不得用模糊概述替代精确数值或后续行，例如“依次递减”“等等”“其他项目类似”。
   - 除非原文明确给出修正，否则不得自行计算、归一化、改名、重排或“纠正”数值和标签。
3. 识别反映文档中事实内容、见解、决策或含义的关键信息——包括任何显著的主题、结论或数据点，使读者无需阅读原文即可充分理解该片段的核心内容。
4. 清晰解析所有时间、人物、地点和事件的指代：
   - 如果上下文允许，将相对时间表达（如“去年”、“下一季度”）转换为绝对日期。
   - 明确区分事件时间和文档时间。
   - 如果存在不确定性，需明确说明（例如，“约2024年”，“具体日期不详”）。
   - 不得编造缺失的年份或日期。只有当文档级上下文能够无歧义地指向同一事件或主题时，才可据此补全年份。
   - 若提及具体地点，请包含在内。
   - 将所有代词、别名和模糊指代解析为全名或明确身份。
   - 如有同名实体，需加以区分。
5. 始终以第三人称视角撰写，清晰指代主题或内容，避免使用第一人称（“我”、“我们”、“我的”）。
6. 不要遗漏文档摘要中可能重要或值得记忆的任何信息。
  ...
  "memory list": [
    {
      ...
      "memory_type": "LongTermMemory",
      "value": <一条清晰、准确、自包含的最小完整记录，并保留紧密相关的字段、精确结构化数值及其对应关系——若输入摘要为英文，则用英文；若为中文，则用中文>,
      "tags": <相关主题关键词列表（例如，["截止日期", "团队", "计划"]）>
    }
    ...
  ],
...
参考上下文只用于消除当前片段中的歧义，例如确定文档、事件、主题或年份。除非当前文档片段本身也支持同一事实，否则不得仅依据参考上下文创建记忆。如果没有上下文，请直接处理文档信息。
参考的上下文：
{context}
