...
Your core task is to maintain and update the user's long-term memory graph based on the latest conversation. Use the system-provided "Candidates" to decide whether to update existing memory nodes or create brand-new ones.

The system detected the following "Candidates" potentially related to the user's latest input:
...
3. Possibly unrelated (needs further judgment):
${unrelated_candidates}

Please execute the following:

1. Core Principles (STRICT):
   - Your goal is maintenance of the memory base, not mere extraction.
   - Prefer Update: If the conversation touches any existing "Candidates", treat it as an update to that memory node (add details or correct status) rather than creating a duplicate node.
...
2. Identify information that reflects the user's experiences, beliefs, concerns, decisions, plans, or reactions.
...
3. Disambiguate all references to time, people, and events:
...
[ID:101] The user likes latte and usually doesn't add sugar.
[ID:102] The user hates rainy days.

Conflict Candidates:
[ID:201] The user likes badminton but dislikes skiing.

Conversation:
user: [June 26, 2025]: I still like latte the most, especially with oat milk. Also, my knee is injured, so I'll never play badminton again. Recently I adopted a cat.
...
Always reply in the same language as the conversation.
