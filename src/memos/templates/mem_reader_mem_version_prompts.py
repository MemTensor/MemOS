...
5. **挽救剩余信息 (Restored Memories)**：
   - `restored_memories` **只能**来自“本次被放入 `conflicted_candidate_ids` 的旧节点”。如果某条候选记忆未被本次冲突更新命中，则**绝不能**放入 `restored_memories`。
   - `restored_memories` 中的每一条内容，都必须能在其 `source_candidate_id` 对应的那条旧记忆原文中直接定位到；它只是“拆分/改写该旧节点内部原本就存在的子事实”，**绝不能**从其他 candidate 挪用、拼接、概括或猜测内容。
   - 当您决定更新一个冲突节点（情况 b）时，请检查该旧节点是否包含**未被新信息否定且依然独立的有效事实**。
   - 只有在“旧冲突节点本身是一条混合记忆，且本次更新只改动其中一部分”时，才返回 `restored_memories`。这能防止因节点更新导致同一旧节点内其他事实丢失。
   - 若旧节点包含多个事实（如“人物状态 + 电话号码”），而新对话仅更新其中一部分，则必须将未被否定的事实拆分为独立条目放入 `restored_memories`，并在冲突更新的 `value` 中仅保留被更新的那一部分。
   - 对于“与本次对话无关的候选记忆”“仅仅没有被提及的旧记忆”“来自 unrelated candidates 的记忆”，一律忽略，不要输出到 `memory list`，也不要输出到 `restored_memories`。
   - 对于可长期独立存在的属性（如电话号码、出生地、所属组织），优先拆分为独立事实，避免与可变状态混写在同一条记忆中。
   - 如果一个 candidate 被判断为无关，请将它的 ID 放入 `unrelated_candidate_ids`，并且不要在 `memory list` 或 `restored_memories` 中再次出现它的内容。
...
5. **Restored Memories**:
   - `restored_memories` may come ONLY from old nodes that you also place into `conflicted_candidate_ids` in this same response. If a candidate is not updated as a conflict this turn, it must NEVER appear in `restored_memories`.
   - Every restored item must be directly traceable to the original text of its own `source_candidate_id`. It is only a split-out/rephrased sub-fact that already exists inside that same old node, and must NEVER borrow, merge, summarize, or infer content from any other candidate.
   - When you decide to update a conflict node (case b), check whether that old node contains valid facts that were NOT negated by the new information and remain independently valid.
   - Return `restored_memories` only when the old conflict node itself is a mixed memory and this update changes only one part of it. This prevents losing the other facts embedded in that same old node.
   ...
   - Completely unrelated candidates, merely unmentioned old memories, and anything from the Unrelated Candidates section must be ignored. Do not output them in `memory list` or `restored_memories`.
   ...
   - If a candidate is judged unrelated, put its ID into `unrelated_candidate_ids`, and do not let its content appear again in `memory list` or `restored_memories`.
