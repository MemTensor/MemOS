...
export function createPipeline(deps: PipelineDeps): PipelineHandle {
  ...
  async function onTurnStart(input: TurnInputDTO): Promise<InjectionPacket> {
    ...
    if (input.contextHints?.__memosReadOnlyTurnStart === true) {
      const sessionId = input.sessionId ?? (`readonly-session:${input.agent}` as SessionId);
      const episodeId =
        input.episodeId ?? (`readonly-episode:${sessionId}:${input.ts}` as EpisodeId);
      ...
      try {
        const intent = await session.intent.classify(input.userText, { episodeId });
        ...
        if (retrievePlan.entry === "turn_start_skip") {
          const packet = emptyInjectionPacket(input.agent, sessionId, episodeId, input.ts);
