...
export interface MemosLocalPlugin {
  ...
  /** Internal: direct store access for bridge/daemon use. */
  _store?: unknown;
}
...
export interface PluginInitOptions {
  ...
  log?: Logger;
  /** Injected model provider API (e.g. OpenClaw host models). Core is agnostic to this. */
...
export function initPlugin(opts: PluginInitOptions = {}): MemosLocalPlugin {
  ...
  const ctx = buildContext(stateDir, workspaceDir, opts.config, opts.log, opts.openclawAPI);
