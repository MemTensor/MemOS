...
class MemosBridgeClient:
    ...
    def request(
        ...
        *,
        ...
        # Fast-fail: if the subprocess has already exited we must not
        # write to a broken pipe. OS-level pipe buffering would happily
        # accept the write, then the caller would park on `waiter.wait`
        # for the full per-request timeout even though the bridge is
        # already dead. Detect that state up front.
    ...
    def close(self) -> None:
        ...
        # 5. Clean up pending requests — shared helper with `_read_loop`.
    ...
    def _read_loop(self) -> None:
        ...
        try:
            ...
            # Any read-side exception (subprocess crash surfaced as OSError,
            # stdout iteration errors) is terminal for the reader thread.
            # We fall through to the finally block so pending waiters are
            # released instead of parking on their per-request timeouts.
            logger.debug("bridge: reader loop terminated by exception", exc_info=True)
        finally:
            # Normal EOF path (bridge subprocess exited cleanly) reaches
            # here without exception. Either way we must wake pending
            # waiters with a transport_closed error — otherwise every
            # in-flight caller sleeps for the full 30 s wall clock and
            # then reports a misleading "[timeout] …" (issue #1722).
            self._abort_pending("bridge subprocess exited")

    def _abort_pending(self, reason: str) -> None:
        """Mark the client closed and wake every parked JSON-RPC waiter.

        Called from both ``_read_loop``'s finally block (subprocess
        died) and ``close`` (client shutting down). Idempotent —
        subsequent calls with the same client are no-ops because
        ``_pending`` is cleared and ``_closed`` is latched.
        """
        with self._host_handlers_cv:
            self._closed = True
            self._host_handlers_cv.notify_all()
