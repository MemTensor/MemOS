...
const en = {
  ...
  "tasks.skillReason.not_generated.belowThreshold":
    "Task score R={rTask} is below the induction threshold (≥ {threshold}) but above the negative-example floor (≤ -0.50); this task has weak or negative signal, so no L2 experience will be generated yet. Similar future tasks can accumulate into reusable experience if they score higher.",
...
const zh: Record<TranslationKey, string> = {
  ...
  "tasks.skillReason.not_generated.belowThreshold":
    "任务评分 R={rTask} 低于沉淀阈值 (≥ {threshold})，但未达到反例阈值 (≤ -0.50)；本次任务信号偏弱或带有负向反馈，暂不会生成 L2 经验。后续相似任务若获得更高评分，会继续累积为可复用经验。",
  "tasks.skillReason.not_generated.noPolicy":
