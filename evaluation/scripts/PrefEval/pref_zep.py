        ...
        if conversation:
            if os.getenv("PRE_SPLIT_CHUNK", "false").lower() == "true":
                for chunk_start in range(0, len(conversation), turns_add * 2):
                    ...
                    mem_client.add(messages=chunk, user_id=user_id, conv_id=None,timestamp=datetime.now().isoformat())
            else:
                mem_client.add(messages=conversation, user_id=user_id, conv_id=None,timestamp=datetime.now().isoformat())
