...
def memos_client(mode="local", db_name=None, user_id=None, top_k=20, mem_cube_path="./", mem_cube_config_path="configs/lme_mem_cube_config.json", mem_os_config_path="configs/mos_memos_config.json", addorsearch="add"):
    """Initialize and return a Memos client instance.""" 
    if mode == "local":
        with open(mem_os_config_path, "r") as f:
        ...
        if addorsearch == "add":
            with open(mem_cube_config_path, "r") as f:
            ...
            # mem_cube_config_data["text_mem"]["config"]["graph_db"]["config"]["user_name"] = (
            #     user_id
            # )        
...
if __name__ == "__main__":
    ...
    result = mem0.add(messages=[{"role": "user", "content": "I like drinking coffee in the morning"},
    ...
    memos_a = memos_client(mode="local", db_name="session333", user_id="dlice", top_k=20, mem_cube_path="./mem_cube_a", mem_cube_config_path="configs/lme_mem_cube_config.json", mem_os_config_path="configs/mos_memos_config.json")
    print("Memos a client initialized successfully.")
    memos_b = memos_client(mode="local", db_name="session444", user_id="alice", top_k=20, mem_cube_path="./mem_cube_b", mem_cube_config_path="configs/lme_mem_cube_config.json", mem_os_config_path="configs/mos_memos_config.json")
    ...
    memos_b.add(
        messages=[{"role": "user", "content": "I like playing football in the evening"},
                  {"role": "user", "content": "I enjoy watching movies at night"}],
        user_id="alice"
    )
    
    # Example of searching memories in Memos
    search_result_a = memos_a.search(
        query="coffee",
        user_id="dlice"
    )
    ...
    search_result_b = memos_b.search(
        query="football",
        user_id="alice"
    )
