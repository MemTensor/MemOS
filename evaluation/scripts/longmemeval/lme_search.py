...
from utils.prompts import ZEP_CONTEXT_TEMPLATE, MEM0_CONTEXT_TEMPLATE, MEM0_GRAPH_CONTEXT_TEMPLATE, MEMOS_CONTEXT_TEMPLATE
...
def mem0_search(client, user_id, query, top_k=20, enable_graph=False, frame="mem0-api"):
    ...
    if frame == "mem0-local":
        ...
        search_memories = "\n".join(
            [f"  - {item['memory']} (date: {item['created_at']})"
            for item in results["results"]]
        )
        search_graph = "\n".join(
    ...
    if enable_graph:
        context = MEM0_GRAPH_CONTEXT_TEMPLATE.format(user_id=user_id, memories=search_memories, relations=search_graph)
    else:
...
def process_user(lme_df, conv_idx, frame, version, top_k=20):
    ...
    user_id = f"lme_exper_user_{str(conv_idx)}"
    id_to_session = {sid: session for sid, session in zip(haystack_session_ids, sessions)}
    answer_sessions = [id_to_session[sid] for sid in answer_session_ids if sid in id_to_session]
    answer_evidences = []
    
    for session in answer_sessions:
        for turn in session:
            if turn.get("has_answer") == True:
    ...
    if frame == "zep":
        client = zep_client()
        print(f"🔌 \033[1mUsing \033[94mZep client\033[0m \033[1mfor search...\033[0m")
        context, duration_ms = zep_search(client, user_id, question)
        
    elif frame == "mem0-local":
        client = mem0_client(mode="local")
        print(f"🔌 \033[1mUsing \033[94mMem0 Local client\033[0m \033[1mfor search...\033[0m")
        context, duration_ms = mem0_search(client, user_id, question, top_k=top_k, frame=frame)
    elif frame == "mem0-api":
        client = mem0_client(mode="api")
        print(f"🔌 \033[1mUsing \033[94mMem0 API client\033[0m \033[1mfor search...\033[0m")
        context, duration_ms = mem0_search(client, user_id, question, top_k=top_k, frame=frame)
        ...
        print(f"🔌 \033[1mUsing \033[94mMemos Local client\033[0m \033[1mfor search...\033[0m")
...
def main(frame, version, top_k=20, num_workers=2):
    ...
    print(f"⚙️  Search parameters: top_k=\033[94m{top_k}\033[0m, workers=\033[94m{num_workers}\033[0m")
    print("-"*80)
    ...
    print("\n" + "="*80)
    print(f"✅ \033[1;32mSEARCH COMPLETE\033[0m".center(80))
    print("="*80)
    print(f"⏱️  Total time taken to search \033[93m{num_multi_sessions}\033[0m users: \033[92m{elapsed_time_str}\033[0m")
    ...
    print(f"📁 Results saved to: \033[1;94mresults/lme/{frame}-{version}/{frame}_lme_search_results.json\033[0m")
    print("="*80 + "\n")
