...
memos_key = os.getenv("MEMOS_KEY", "Token mpg-MzKpSDXtEcueHEtlXfDwcZ0vRkYiHYjudBwgVl4l")
memos_url = os.getenv("MEMOS_URL", "http://0.0.0.0:8009")
...
OUTPUT_FILE = "./Personamem_memos.jsonl"
...
class MemuClient:
    def __init__(self):
        ...
        self.memu_client = MemuClient(
            base_url="https://api.memu.so", api_key="mu_akbVF6lyrdmei8rFAmcf6JORw5klQ7_51Mr64I1V-dwJpM-kQbvG9wR7uwRkI3KSelhCuWrr9Kd4ljy0rxiCLlQ7IYB7tTfiYUoo3A"
        )
...
def process_line(line_data: tuple, mem_client: MemuClient, openai_client: OpenAI) -> dict:
    i, line = line_data
    ...
    try:
        ...
        # 添加对话历史到 MemOS - 分成多个块添加
        CHUNK_SIZE = 20  # 将对话分成4个块
        chunk_size = max(1, len(conversation) // CHUNK_SIZE)  # 确保至少为1
