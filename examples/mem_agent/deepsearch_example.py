...
from memos.configs.mem_agent import DeepSearchAgentConfig, MemAgentConfigFactory
...
def example_1_factory_initialization():
    ...
    deep_search_agent = MemAgentFactory.from_config(
        config_factory=agent_config_factory,
        llm=llm,
        memory_retriever=naive_mem_cube.text_mem
    )
    
    logger.info(f"✓ DeepSearchMemAgent created successfully")
...
def example_3_usage(deep_search_agent, components):
    ...
    test_memories = [
        {
            "user_name": "test_user",
            "messages": ["Artificial Intelligence is a branch of computer science dedicated to creating systems capable of performing tasks that typically require human intelligence."],
            "source": "manual"
        },
        {
            "user_name": "test_user", 
            "messages": ["Machine Learning is a subfield of artificial intelligence that enables computers to learn from data and improve performance."],
            "source": "manual"
        },
        {
            "user_name": "test_user",
            "messages": ["Deep Learning is a branch of machine learning that uses multi-layer neural networks to handle complex pattern recognition tasks."],
            "source": "manual"
        }
    ]
...
def example_4_minimal_initialization():
    ...
    logger.info(f"✓ DeepSearchMemAgent created successfully (using default configuration)")
    logger.info(f"  - Max iterations: {deep_search_agent.max_iterations}")
    logger.info(f"  - Timeout: {deep_search_agent.timeout} seconds")
