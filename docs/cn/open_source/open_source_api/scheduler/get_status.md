...
当前 Python SDK 仅封装了公开的 `/get/status` 任务查询方法；调度器状态接口需要按 HTTP API 直接调用。以下示例使用 `requests` 轮询任务状态直至完成：
...
global_res = requests.get(f"{base_url}/scheduler/allstatus", headers=headers, timeout=30)
global_res.raise_for_status()
print(f"系统运行概况: {global_res.json()['data']['scheduler_summary']}")

# 2. 队列指标监控：检查特定用户的任务积压情况
queue_res = requests.get(
    f"{base_url}/scheduler/task_queue_status",
    params={"user_id": user_id},
    headers=headers,
    timeout=30,
)
queue_res.raise_for_status()
queue_data = queue_res.json()["data"]
print(f"待处理任务数: {queue_data['remaining_tasks_count']}")
print(f"已下发未完成任务数: {queue_data['pending_tasks_count']}")
...
while True:
    ...
    tasks = status_res.json()["data"]
    current_status = tasks[0]["status"] if tasks else "completed"
